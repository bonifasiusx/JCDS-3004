...
headers = ['Index', 'Nama', 'Stok', 'Harga'] # Column List PRODUCT
...
def exit(): # [0] Exit Program
    closing = ''
    closing_msg = ['PROGRAM SELESAI, TERIMAKASIH']
    print()
    print(tabulate(closing, closing_msg, tablefmt='grid', stralign='center'))
    return

def showBuah(): # [1] Menampilkan Daftar Buah    
...
def addBuah(): # [2] Menambah Buah
    ...
    if not add_price.isdigit() or not add_stock.isdigit():
        ...
        return add_product, add_stock, add_price
    else:
        # Membuat list baru
...
def transactionDetails():
    ...
    if uang < buyBuah.total_price:
        # Skenario Transaksi GAGAL sampai kondisi uang > total_price tercapai
        print('\nTransaksi GAGAL')
        print(f'Pembayaran diterima: Rp {uang}')
        print(f'Kekurangan pembayaran sebesar: Rp {buyBuah.total_price - uang}\n')
        
        uang_tambahan = int(input('Masukkan Dana Tambahan: '))
        uang += uang_tambahan # Selalu update jumlah uang di dalam loop
    
        if uang > buyBuah.total_price: # Keluar dari while loop uang < total_price
            return
        else:
            return uang # Transaksi GAGAL sampai kondisi uang > total_price tercapai
    
    if uang > buyBuah.total_price: # Jika uang > total_price, kasih kembalian
        kembalian = uang - buyBuah.total_price
        print('\nTransaksi Selesai!')
